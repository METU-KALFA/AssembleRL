...
""" Taken from https://github.com/clvrai/furniture/blob/master/furniture/env/transform_utils.py """
...
def _get_pos_quat(mat, to="xyzw"):
    pos_quat = mat2pose(mat)
    if to == "xyzw":
        pos_quat = np.concatenate((pos_quat[0], pos_quat[1]))
    elif to == "wxyz":
        pos_quat = np.concatenate((pos_quat[0], convert_quat(pos_quat[1], "wxyz")))
    return pos_quat

def _compare_pos_list(pos_list, new_pos, err):
    for pos in pos_list:
        dist = _compute_distance(pos, new_pos)
        if dist < err: return True
    return False

def relative_pose_from_poses(obj1_pose_in_world, obj2_pose_in_world):
    world_pose_in_obj1 = pose_inv(obj1_pose_in_world)
    obj2_pose_in_obj1 = pose_in_A_to_pose_in_B(obj2_pose_in_world, world_pose_in_obj1)
    return obj2_pose_in_obj1
